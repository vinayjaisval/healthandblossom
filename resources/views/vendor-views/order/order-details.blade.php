                                ...
                                @foreach($order->details as $key=>$detail)
                                    ...
                                    @if($productDetails)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="media align-items-center gap-10">
                                                    ...
                                                    <div>
                                                      ...
                                                      <div>
                                                        <form action="{{ route('vendor.orders.hnbinvoice', ['order_id' => $detail['order_id'], 'primary_id' => $detail['id']]) }}" method="POST" style="display:inline;">
                                                            @csrf
                                            ...
                                            <td>
                                            <select class="form-control text-capitalize orderAction_delivery_status" name="delivery_status" data-id="{{$detail['id']}}" data-third_party_delivery_tracking_id-id="{{$detail['third_party_delivery_tracking_id']}}">
...
<script>
  function myFunction(id) {
     ...
     $.ajax({
         url: "{{ route('vendor.orders.printslip') }}",
         type: 'POST',
  ...
  $('.orderAction_delivery_status').on('change', function() {
            ...
            $.ajax({
                url: "{{ route('vendor.orders.return-cancel') }}", // Ensure there are no extra spaces
                type: "POST",
