...
$("#toggle-organic-modal-ok-button").on("click", function () {
    const toggleId = $("#" + $(this).attr("toggle-ok-button"));
    if (toggleId.is(":checked")) {
        toggleId.prop("checked", false);
    } else {
    ...
    let toggleOkButton = $(this).attr("toggle-ok-button") + "-form";
    checkAlternativeCheckbox($(this).attr("toggle-ok-button"));
    submitStatusOrganicUpdateForm(toggleOkButton);
});

function submitStatusOrganicUpdateForm(formId) {
    const form = $("#" + formId);
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
    ...
    console.log("this is trets",form.attr("action"));
    let updateText = $("#get-update-organic-message");
    $.ajax({
