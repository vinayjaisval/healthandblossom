...
class OrderController extends BaseController
{
    ...
    ): JsonResponse
    {
        ...
        if ($request->order_status == 'canceled' || $request->order_status=='confirmed') {
  	...
  	public function cancelWaybill($id){
        $order = OrderCancel::where('id',$id)->first();
        $client = new Client();
    ...
    public function hnbinvoice(Request $request, $order_id, $detail_id){
          ...
          $invoiceSettings = json_decode(json: $this->businessSettingRepo->getFirstWhere(params: ['type'=>'invoice_settings'])?->value);
           //return view('vendor-views.order.hnbinvoice', compact(
           //    'order', 'vendor', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo', 'invoiceSettings','detail_id'
          // ));
  ...
  public function cancelWaybill2(Request $request)
    {
        ...
        Log::info('Cancel Waybill Request:', [
        ...
        try {
            ...
            $content = $response->getBody()->getContents();
            // Log the response
            Log::info('Cancel Waybill Response:', [
                'status_code' => $statusCode,
            ...
            // Log the error
            Log::error('Cancel Waybill Error:', [
                'error' => $e->getMessage(),
            ]);
            return response()->json([
                'error' => $e->getMessage(),
