...
class ProductController extends BaseController
{
    ...
    public function getAddView(): View
    {
        ...
        return view(Product::ADD[VIEW], compact('languages','ingredients','categories', 'brands', 'brandSetting', 'digitalProductSetting', 'colors', 'attributes', 'languages', 'defaultLanguage', 'digitalProductFileTypes'));
    }
    ...
    public function add(ProductAddRequest $request, ProductService $service): JsonResponse|RedirectResponse
    {
        ...
        //add by ankit for wholsaler
          $wholsalerData = [];
          if (isset($request['min-qty']) && is_array($request['min-qty'])) {
              foreach ($request['min-qty'] as $key => $minQty) {
                  $wholsalerData[] = [
                      'min_qty' => $minQty,
                      'max_qty' => $request['max-qty'][$key] ?? 0,
                      'price'   => $request['price'][$key] ?? 0,
                  ];
              }

              $this->wholsalerRepo->add(wholsalerData: $wholsalerData, productId: $savedProduct->id ?? 0);
          }
    ...
    public function update(ProductUpdateRequest $request, ProductService $service, string|int $id): JsonResponse|RedirectResponse
    {
        ...
        if (isset($request['min-qty']) && is_array($request['min-qty'])) {
            ...
            $this->wholsalerRepo->add(wholsalerData: $wholsalerData, productId: $request['product_id'] ?? 0);
        }
