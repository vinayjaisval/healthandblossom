...
class LanguageService
{
    ...
    public function getAddDataold(object $request, object $language): array
    {
        $languageArray = [];
        $codes = [];
  ...
  public function getAddData(object $request, object $language): array
{
    ...
    // ✅ Extra strict validation for language code (e.g. en, fr)
    ...
    $langPath = base_path('resources/lang/' . $request['code']);
    if (!file_exists($langPath)) {
        mkdir($langPath, 0755, true);
    }

    // ✅ Load source language safely
    $defaultMessages = include(base_path('resources/lang/en/messages.php'));
    $defaultNewMessages = include(base_path('resources/lang/en/new-messages.php'));

    // ✅ Merge safely
    $allMessages = array_merge(
        is_array($defaultMessages) ? $defaultMessages : [],
        is_array($defaultNewMessages) ? $defaultNewMessages : []
    );

    // ✅ CLEAN + SANITIZE DATA (CRITICAL FIX)
    $dataFiltered = [];
    ...
    foreach ($allMessages as $key => $value) {
        ...
        $dataFiltered[$cleanKey] = $cleanValue;
    }

    // ✅ Generate safe PHP file content
    $fileContent = "<?php\n\nreturn " . var_export($dataFiltered, true) . ";\n";

    // ✅ Write files safely (LOCK_EX prevents race conditions)
    file_put_contents($langPath . '/new-messages.php', $fileContent, LOCK_EX);

    // create empty messages.php
    file_put_contents(
        $langPath . '/messages.php',
        "<?php\n\nreturn [];\n",
        LOCK_EX
    );
